...
export default function UserDashboard() {
  ...
  return (
    <div className={`max-w-7xl mx-auto px-2 sm:px-4 md:px-6 lg:px-8 py-8 sm:py-12 bg-[var(--background)] min-h-[85vh] ${styles.dashboardContainer}`}>
      ...
      <div className="mt-10 sm:mt-16 flex flex-col md:flex-row items-center justify-between bg-[var(--background)] rounded-xl shadow-lg p-6 sm:p-8 border border-[var(--border)] gap-4 md:gap-0">
        <div className="mb-2 md:mb-0 text-center md:text-left w-full md:w-auto">
          <h2 className="text-lg sm:text-2xl font-bold text-[var(--primary)] mb-1">Ready to improve your next pitch?</h2>
          <p className="text-[var(--foreground)] text-sm sm:text-base">Record or upload a new audio sample to get instant feedback and insights.</p>
        </div>
        <Link href="/record" className="w-full md:w-auto">
          <button className="w-full md:w-auto bg-gradient-to-r from-[var(--accent)] to-[var(--secondary)] text-[var(--background)] px-6 sm:px-8 py-3 rounded-lg font-semibold text-base sm:text-lg shadow-md hover:opacity-90 transition-all duration-200">
            Start Now
          </button>
        </Link>
      </div>
